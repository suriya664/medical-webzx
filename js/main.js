...
$(document).ready(function() {
    ...
    function setActiveNavLink() {
        ...
        // Check dropdown items first (more specific)
        ...
        $('.navbar-nav > .nav-item > .nav-link:not(.dropdown-toggle)').each(function() {
            const linkHref = $(this).attr('href');
            if (linkHref === currentPage || (currentPage === '' && linkHref === 'index.html')) {
                $(this).addClass('active');
            }
        ...
        if (currentPage === 'index.html' || currentPage === '' || currentPage === 'index-2.html') {
            $('.navbar-nav .nav-link[href="index.html"], .navbar-nav .dropdown-item[href="index.html"]').addClass('active');
            $('.navbar-nav .nav-link[href="#"]').filter(function() {
                return $(this).closest('.dropdown').find('.dropdown-item[href="index.html"]').length > 0;
            }).addClass('active');
        }
    }
    setActiveNavLink();
    ...
    $mobileMenuToggle.on('click', function(e) {
        ...
        // Ensure body is never blocked
        ensureBodyNotBlocked();
    });
    
    // Close mobile menu when clicking outside the menu panel
    $(document).on('click', function(e) {
        // Check if click is outside both menu panel and toggle button
        if (!$(e.target).closest('.mobile-menu-panel').length && 
    ...
    $('.mobile-menu-nav .nav-link:not(.dropdown-toggle)').on('click', function(e) {
    ...
    $('.mobile-menu-nav .dropdown-item').on('click', function(e) {
        const href = $(this).attr('href');
        if (href && href !== '#' && !href.startsWith('javascript:')) {
            setTimeout(function() {
                $mobileMenuToggle.removeClass('active');
    ...
    $('.mobile-menu-auth .btn').on('click', function(e) {
        const href = $(this).attr('href');
        if (href && href !== '#' && !href.startsWith('javascript:')) {
            setTimeout(function() {
                $mobileMenuToggle.removeClass('active');
    ...
    // Toggle dropdown menus in mobile menu
    $('.mobile-menu-nav .nav-link.dropdown-toggle').on('click', function(e) {
        e.preventDefault();
        e.stopPropagation();
        
        const $dropdown = $(this).next('.dropdown-menu');
        const $allDropdowns = $('.mobile-menu-nav .dropdown-menu');
        const $allToggles = $('.mobile-menu-nav .nav-link.dropdown-toggle');
